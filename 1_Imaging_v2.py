...
def read_pd_parquet(location):
    ...
    return read
def Preprocess_Refinitiv(pvt_tmp):
    pvt_tmp.columns = pvt_tmp.iloc[0]
    pvt_tmp = pvt_tmp.iloc[1:]
    pvt_tmp = pvt_tmp.set_index('Code').rename_axis('date')
    pvt_tmp.index = pd.to_datetime(pvt_tmp.index)
    pvt_tmp = pvt_tmp[pvt_tmp.columns[~pvt_tmp.columns.isna()]]
    pvt_tmp = pvt_tmp.rename(columns=lambda x: re.sub(r'\(.*?\)', '', x).replace(':', '_'))
    pvt_tmp = pvt_tmp.dropna(how='all', axis=0).dropna(how='all', axis=1)
    return pvt_tmp
...
def process_params_v5(code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict):
    # code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict = year_price['close'].columns[10], year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict
    # code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict = 'A071530', year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict
    # code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict = 'A005930', year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict
    # code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict = 'A000480', year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict
    # code = i
    local_params_list = []
    ...
    # if len(price_a_stock.dropna(how='all', axis=0))==0:
    #     return []
    ...
    for idx in range(from_day_type_int * 2 + 1, len(price_a_stock.loc[first_date:last_date].index) + 1):
        input_df = price_a_stock.iloc[idx - from_day_type_int * 2:idx]
        if len(input_df.dropna(how='all', axis=0)) == 0:
            continue
        end_dt = input_df.index[-1]
        # if (ExPost_ret_code.dropna().index[-1] < end_dt)&(input_df.index[-1].year == NaN_year):
        # continue
        # print(f'NANANN code: {code}')
        # print(f'{input_df}')
        # label = np.nan
        # else:
        try:
            ...
            label = -1
        input_df = price_a_stock.iloc[idx - from_day_type_int * 2:idx].reindex(
            ['open', 'high', 'low', 'close', 'volume', 'C_MA', 'O_MA', 'V_MA'], axis=1)
        ...
        open_to_close_time = pd.concat([open_time, close_time], axis=0).sort_index(); C_MA_tmp = open_to_close_time.rolling(min_periods=to_day_type_int * 2, window=to_day_type_int * 2).mean().loc[lambda x: x.index.hour == 16]
        ...
        V_MA_tmp = transformed_input.volume.rolling(min_periods=to_day_type_int, window=to_day_type_int).mean()
        C_MA_tmp.index = pd.to_datetime(C_MA_tmp.index.strftime("%Y-%m-%d"))
        O_MA_tmp.index = pd.to_datetime(O_MA_tmp.index.strftime("%Y-%m-%d"))
        V_MA_tmp.index = pd.to_datetime(V_MA_tmp.index.strftime("%Y-%m-%d"))
        transformed_input['C_MA'] = C_MA_tmp
        transformed_input['O_MA'] = O_MA_tmp
        transformed_input['V_MA'] = V_MA_tmp
        ...
        transformed_input = transformed_input.iloc[-to_day_type_int - 1:]

        if (len(transformed_input.dropna(how='all', axis=0)) < to_day_type_int) or (
                len(transformed_input.dropna(how='all', axis=1).columns) < len(transformed_input.columns)):
            continue
        local_params_list.append((transformed_input, code, end_dt, label, to_day_type_int, image_size_dict, from_day_type_int))
...
def process_params(code, year_price, ExPost_return, from_day_type_int, to_day_type_int, image_size_dict, only_today=False):
    ...
    for idx in range(stt_idx, len(price_a_stock.loc[first_date:last_date].index)+1):
        ...
        try:
            ...
            label=-1
        input_df = input_df.reindex(['open', 'high', 'low', 'close', 'volume', 'C_MA', 'O_MA', 'V_MA'], axis=1)
        chunk_size = from_day_type_int//to_day_type_int
...
def get_changes_quantized(x, n_day):
    x = x * (n_day // 5)
    if x >= 0.10:
        output = [255, 0, 0]  # 36
    elif (x < 0.10) & (x >= 0.08):
        ...
        output = [0, 0, 0]
    return output
def CancleVolume_with_Trend_imaging(params):
    # params = pp
    # params = params_list[10]
    input_df, code, end_dt, label, day_type, image_size_dict, n_day = params
    # df, height, interval = input_df.copy(), image_size_dict[day_type][0], image_size_dict[day_type][1]
    img = convert_ohlcv_to_CancleVolume_with_Trend(input_df, image_size_dict[day_type][0], image_size_dict[day_type][1], n_day)
    return img, code, end_dt.strftime("%Y%m%d"), label
def convert_ohlcv_to_CancleVolume_with_Trend(df, height, interval, n_day):
    df['prc_RGB'] = df['close'].div(df['open']).sub(1).apply(lambda x: get_changes_quantized(x, n_day))
    df['vlm_RGB'] = df['volume'].pct_change().apply(lambda x: get_changes_quantized(x, n_day))
    df = df.iloc[1:]
    RGB_Green = [0, 255, 0]

    # 원하는 데이터 크기의 행렬을 만들어 zero(black)를 다 깔아놓고 (0:black, 255:white)
    img = np.zeros((height, 3 * interval, 3), dtype=np.uint8)

    # Price scaling
    min_price = np.nanmin([df['close'].min(), df['open'].min(), df['low'].min(), df[f'C_MA'].min(), df[f'O_MA'].min()])
    max_price = np.nanmax([df['close'].max(), df['open'].max(), df['high'].max(), df[f'C_MA'].max(), df[f'O_MA'].max()])
    max_volume = np.nanmax([df['volume'].max(), df['V_MA'].max()])
    ...
    if max_price - min_price != 0:
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            if not np.isnan(row['open']):
                open_pixel = int(np.round((row['open'] - min_price) * price_scale))
                img[-int((height * 1 / 5) + open_pixel), 3 * i] = row['prc_RGB']
            if not np.isnan(row['close']):
                close_pixel = int(np.round((row['close'] - min_price) * price_scale))
                img[-int((height * 1 / 5) + close_pixel), 3 * i + 2] = row['prc_RGB']
            if not np.isnan(row['high']) and not np.isnan(row['low']):
                high_pixel = int(np.round((row['high'] - min_price) * price_scale))
                low_pixel = int(np.round((row['low'] - min_price) * price_scale))
                img[-int((height * 1 / 5) + high_pixel):-int((height * 1 / 5) + low_pixel) + 1, 3 * i + 1] = row[
                    'prc_RGB']
            if not np.isnan(row['volume']):
                volume_pixel = int(np.round(row['volume'] * volume_scale))
                if volume_pixel != 0:
                    img[-int(volume_pixel):, 3 * i + 1] = row['vlm_RGB']

            if not np.isnan(row[f'O_MA']):
                Oma_pixel = int(np.round((row[f'O_MA'] - min_price) * price_scale))
                img[-int((height * 1 / 5) + Oma_pixel), 3 * i] = img[-int(
                    (height * 1 / 5) + Oma_pixel), 3 * i] + RGB_Green
            if not np.isnan(row[f'C_MA']):
                Cma_pixel = int(np.round((row[f'C_MA'] - min_price) * price_scale))
                img[-int((height * 1 / 5) + Cma_pixel), 3 * i + 2] = img[-int(
                    (height * 1 / 5) + Cma_pixel), 3 * i + 2] + RGB_Green
            if not np.isnan(row[f'C_MA']) and not np.isnan(row[f'O_MA']):
                Oma_pixel = int(np.round((row[f'O_MA'] - min_price) * price_scale))
                Cma_pixel = int(np.round((row[f'C_MA'] - min_price) * price_scale))
                ma_pixel = int((Oma_pixel + Cma_pixel) * 0.5)
                img[-int((height * 1 / 5) + ma_pixel), 3 * i + 1] = img[-int(
                    (height * 1 / 5) + ma_pixel), 3 * i + 1] + RGB_Green
            if not np.isnan(row['V_MA']):
                Vma_pixel = int(np.round(row['V_MA'] * volume_scale))
                if Vma_pixel != 0:
                    img[-int(Vma_pixel), 3 * i:3 * i + 3] = img[-int(Vma_pixel), 3 * i:3 * i + 3] + RGB_Green
        return img
    else:
        return None
if __name__ == '__main__':
    ...
    for year in years[::-1]:
        for from_day_type, to_day_type in [[5, 5]]: #[20, 20]
            ...
            results = parmap.map(CancleVolume_with_Trend_imaging, params_list, pm_parallel=True, pm_processes=num_cores, pm_pbar=True)
