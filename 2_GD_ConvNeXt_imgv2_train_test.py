...
class ConvNeXtBlock(nn.Module):
    def __init__(self, dim, drop_prob=0.):
        super().__init__()
        self.dw = nn.Conv2d(dim, dim, kernel_size=(7,3), padding=(3,1), groups=dim)
    ...
    def forward(self, x):
        h = self.dw(x)
        h = self.ln(h)
        h = self.pw1(h); h = self.act(h); h = self.pw2(h)
...
class ConvNeXt_shallow(nn.Module):
    ...
    def __init__(self, in_ch=1, num_classes=2, blocks=2, base_dim=64,
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(in_ch, base_dim, kernel_size=4, stride=2, padding=1),
...
class ConvNeXtLite(nn.Module):
    """
    2-Stage ConvNeXt (dim = 48→96, depth=2,2)  약 2.6 M param
    """
    def __init__(self, in_ch=1, num_classes=2, base_dim=48,
                 depths=(2,2), drop_path_rate=0.1, dr_rate=0.3):
        super().__init__()
        # ① Patchified Stem
        self.stem = nn.Sequential(
            nn.Conv2d(in_ch, base_dim, 4, 2, 1),  # HW/2
            nn.GroupNorm(1, base_dim)
        )

        dims = [base_dim, base_dim*2]
        # ② Build stages
        dp_rates = torch.linspace(0, drop_path_rate, sum(depths)).tolist()
        cur = 0
        self.stages = nn.ModuleList()
        for i, (dpt, dim) in enumerate(zip(depths, dims)):
            blocks = [ConvNeXtBlock(dim, dp_rates[cur+j]) for j in range(dpt)]
            self.stages.append(nn.Sequential(*blocks))
            cur += dpt
            if i < len(depths)-1:   # Downsample & channel 2×
                self.stages.append(
                    nn.Sequential(
                        nn.Conv2d(dim, dims[i+1], 2, 2),
...
def build_optimizer(model, lr=1e-3, wd=0.05):
    ...
    for n,p in model.named_parameters():
        if p.requires_grad:
            if p.ndim==1 or n.endswith(".bias"):  # γ, β, bias
                no_decay.append(p)
            else: decay.append(p)
    return AdamW(
...
if __name__ == '__main__':
    ...
    # ───── grid candidates ─────
    base_dim_list = [32, 48, 64]  # stem/channel scale
    depths_list = [(2, 2), (3, 3), (3, 4)]  # stage depths
    drop_path_rate_list = [0.0, 0.1, 0.2]  # Stochastic-Depth prob
    dr_rate_list = [0.2, 0.5]  # Dropout prob (fixed)
    lr_list = [1e-3, 5e-4, 3e-4]  # base learning-rate
    ...
    for base_dim in base_dim_list[::-1]:
        for depths_idx, depths in enumerate(depths_list, start=1):
            for drop_path_rate in drop_path_rate_list[::-1]:
                for dr_rate in dr_rate_list[::-1]:
                    for lr in lr_list[::-1]:
                        lr_tag = f"{lr:.0e}".replace('-0', '-')
                        bCNN_050505_mdl_pth = f"{model_save_path}/BD{base_dim}_Dpt{depths_idx}_DrPthR{drop_path_rate}_LR{lr_tag}.pt"
    ...
    # output_df.to_excel('./ConvNeXtLite_grid_search_results.xlsx', index=False)